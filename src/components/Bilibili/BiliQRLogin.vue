...
const button_click = () => {
  ...
  requests
    ...
    .then((resp) => {
      ...
      qr_timeout_time.value = formatDate_ZeroFill(
        resp.data.data.time + 180 * 1000
      );
      qr_check(resp.data.data.auth_code);
    })
    .catch((err) => {
    ...
    <div id="bili_qr_content" :style="{ height: '500px' }">
      ...
      <a-space direction="vertical" align="center" v-if="qr_current == 2">
        ...
        <a-space direction="vertical" v-if="qr_status == 'error'">
          <a-result status="error" title="获取失败">
            <template #subtitle> 二维码已过期 </template>
            <template #extra>
              <a-space>
                <a-button
                  type="primary"
                  @click="button_click"
                  v-if="qr_status == 'error'"
                  >重新获取</a-button
                >
              </a-space>
            </template>
      ...
      <a-space direction="vertical" align="center" v-if="qr_current == 3">
        <a-result status="success" title="获取成功">
          <template #subtitle> Cookie获取成功 </template>
        </a-result>
      </a-space>
    </div>
  </a-card>
