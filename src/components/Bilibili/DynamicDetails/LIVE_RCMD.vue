...
const data = JSON.parse(props.data.modules.module_dynamic.major.live_rcmd.content).live_play_info
function go(room_id){
    let url = 'https://live.bilibili.com/'+room_id;
    window.open('javascript:window.name;', '<script>location.replace("'+url+'")<\/script>');
}
const live_start_time = dateFormatter(new Date(data.live_start_time*1000))
</script>
